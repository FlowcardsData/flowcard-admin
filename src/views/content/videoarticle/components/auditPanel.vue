...
<!--          </el-button>-->
<!--          <el-button size="small" type="success" class="ele-btn-icon" @click="openBatchReviewDialog()"-->
<!--                     v-if="permission.includes('sys:video:dall') ">批量审核-->
<!--          </el-button>-->
        </template>
        <!-- 操作列 -->
        <template slot="action" slot-scope="{row}">
          ...
          <el-link type="success" slot="reference" :underline="false" icon="el-icon-check" @click="openDialog(row.id)"
             v-if="permission.includes('sys:video:agree') && row.status == 2">审核
          </el-link>
    ...
    <BatchReview :visible.sync="batchReviewVisible" :ids="batchReviewIds" @cancel="closeBatchReviewDialog" />
  </div>
</template>
...
import BatchReview from "./batchReview.vue";
...
export default {
  ...
  data() {
    return {
      showType: '2',
      // 表格搜索条件
      where: {'video_type':2, 'show_type':2},
      dialogVisible: false,
