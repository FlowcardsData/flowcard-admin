<template>
  <div class="panel">
    <el-card shadow="never">
        ...
        <el-row :gutter="15">
          ...
          <el-col :lg="6" :md="12">
            <el-form-item label="用户ID:">
              <el-input clearable v-model="where.uid" placeholder="请输入用户ID" />
            </el-form-item>
          </el-col>
          <el-col :lg="6" :md="12">
      ...
      <ele-pro-table ref="table" :where="where" :datasource="url" :columns="columns" :selection.sync="selection"
        ...
        <template slot="toolbar">
          <!-- <el-button size="small" type="primary" icon="el-icon-plus" class="ele-btn-icon" @click="openEdit(null)" v-if="permission.includes('sys:video:add')">添加
          </el-button> -->
        ...
        <template slot="action" slot-scope="{row}">
          ...
          <el-popconfirm class="ele-action" title="确定要删除此视频吗？" @confirm="forceDelete(row)"
                         v-if="permission.includes('sys:video:delete') && row.mark==0">
            <el-link type="danger" slot="reference" :underline="false" icon="el-icon-delete">彻底删除
            </el-link>
          </el-popconfirm>
          <el-popconfirm class="ele-action" title="确定要恢复此视频吗？" @confirm="recovery(row)">
            <el-link type="primary" slot="reference" :underline="false" icon="el-icon-refresh">恢复
            </el-link>
          </el-popconfirm>
        </template>
        ...
        <template slot="title" slot-scope="{row}">
          <el-tag size="small" v-show="row.is_good == 1">商品</el-tag>{{ row.title }}
        </template>
        ...
        <!-- <template slot="status" slot-scope="{row}">
          <el-tag :type="['success', 'primary', 'warning', 'info'][row.status-1]" size="mini">
            {{ videoStatusFormat(row) }}
          </el-tag>
        </template> -->
        <template slot="status">
          <el-tag type="warning" size="small">审核驳回</el-tag>
        </template>
        <!-- 切片系统视频状态 -->
        <template slot="section_status" slot-scope="{row}">
          <el-tag v-if="row.href_w" style="cursor: pointer;"
                  :type="['success', 'primary', 'warning', 'info'][row.section_status - 1]" @click="clickPlay(row.href_w)"
                  size="mini">
            {{ sectionStatusFormat(row) }}
          </el-tag>
          <el-tag v-else :type="['success', 'primary', 'warning', 'info'][row.section_status - 1]" size="mini">
            {{ sectionStatusFormat(row) }}
          </el-tag>
        </template>
        <template slot="href" slot-scope="{ row }">
          <el-tag v-if="row.href" class="pointer" type="success" @click="clickPlay(row.href)">
            已上传
          </el-tag>
          <el-tag v-else type="danger">未上传</el-tag>
        </template>
        <template slot="href_w" slot-scope="{ row }">
          <el-tag v-if="row.href_w" class="pointer" type="success" @click="clickPlay(row.href_w)">
            已转码
          </el-tag>
          <el-tag v-else type="danger">未转码</el-tag>
        </template>
        <template slot="href_free" slot-scope="{ row }">
          <el-tag v-if="row.href_free" class="pointer" type="success" @click="clickPlay(row.href_free)">
            已转码
          </el-tag>
          <el-tag v-else type="danger">未转码</el-tag>
        </template>
    ...
    <playVideoDialog v-if="isPlayDialog" :info-data="infoData" :play-url="playUrl" :visible="isPlayDialog" @close="closePlay" />
    <!-- 详情弹窗 -->
...
export default {
  ...
  components: {detail, playVideoDialog},
  props: {
  ...
  data() {
    return {
      ...
      showType: '68',
      // 表格搜索条件
      where: {'video_type':2, 'show_type':68},
      dialogVisible: false,
  ...
  methods: {
    ...
    forceDelete(row) {
      const loading = this.$loading({ lock: true });
      this.$http.post('/video/forceDelete', { id: row.id }).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.reload();
    ...
    recovery(row) {
      const loading = this.$loading({ lock: true });
      this.$http.post('/video/recoveryDelete', { id: row.id }).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.reload();
