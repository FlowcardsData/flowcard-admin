...
<!--          </el-button>-->
        ...
        <template slot="action" slot-scope="{row}">
          ...
          <el-popconfirm class="ele-action" title="确定推荐此视频吗？" @confirm="recomd(row, 1)"
                         v-if="permission.includes('sys:video:agree') && row.status == 3">
            <el-link type="success" slot="reference" :underline="false" icon="el-icon-_like">推荐
            </el-link>
          </el-popconfirm>
        ...
        <template slot="status">
          <el-tag type="info" size="small">仅用户自己可见</el-tag>
        </template>
      </ele-pro-table>
      <!-- 详情弹窗 -->
...
export default {
  ...
  data() {
    return {
      ...
      showType: '70',
      // 表格搜索条件
      where: {'video_type':2, 'show_type':70},
      dialogVisible: false,
