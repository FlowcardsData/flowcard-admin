<template>
  <el-dialog :destroy-on-close="true" :lock-scroll="false" :title="isUpdate ? '修改代理' : '添加代理'" :visible="visible"
             width="880px" @update:visible="updateVisible">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px">
      <el-row :gutter="15">

        <el-form-item label="等级：" prop="agent_level">
          <el-select filterable clearable v-model="form.agent_level" size="small" placeholder="请选择等级" class="ele-block">
            <el-option v-for="item in agentLevelOptions" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>

        <el-form-item label="选择用户:" prop="extend_offset_coins">
          <el-tag v-if="this.selectMemberData">
            {{ selectMemberData.nickname }}
          </el-tag>
          <el-button style="margin-left: 10px" type="success" class="ele-btn-icon" @click="openSelectMember()">请选择</el-button>
        </el-form-item>
    ...
    <SelectMember
      v-if="!!this.showSelectMember"
      @update:gift_info="handleCustomEvent"
      :showGift.sync="showSelectMember"
      :form_info="{ ...this.form, type: this.showSelectMember }" />
  </el-dialog>
  <!-- 选择商品窗口 -->
</template>
